    ...
    public void run() {
        ...
        while (number >= 0) {
            while (!queue[size].isEmpty()) {
                ...
                statusTable.setValueAt("运行态", pcb.psw, 1);
                if (pcb.pri != 0) {
                    statusTable.setValueAt(--pcb.pri, pcb.psw, 2);
                }

                statusTable.setValueAt((arraylist.get(pcb.psw).total - arraylist.get(pcb.psw).time) * 100 / arraylist.get(pcb.psw).total, pcb.psw, 5);
                ...
                statusTable.setValueAt("就绪态", pcb.psw, 1);
                if (pcb.time == 0) {
                    statusTable.setValueAt("终止态", pcb.psw, 1);
                } else if (pcb.pri == 0) {
                    queue[0].offer(pcb);
                } else {
